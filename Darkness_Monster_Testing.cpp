...
bool test_Darkness_Monster() {
    ...
    Closet_Monster cm3(25);
    if(!(cm3.poke_eye())){
        passed = false;
        cout << "FAILED normal test case for poke_eye" << endl;
    }
    return passed;
}
...
bool test_Under_Bed_Monster() {
    ...
    string s = "*" + to_string(ubm3.get_number_of_eyes());
    s += " eyes watching you*";
    if(ubm3.scare() != "*Grabs you*" && ubm3.scare() != s){
        passed = false;
        cout << "FAILED scare test returning irregualar value" << endl;
    }
    return passed;
}